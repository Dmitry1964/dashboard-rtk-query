...
import { IMenuMode, IPartnerFormData } from 'src/app/app-types';
...
const PartnerInfo = ({ mod }: PartnerInfoProps) => {

  const { contacts, general } = mod;
  ...
  const cangeFormData = useCallback((data: RootCompany) => {
    setFormData(prev => ({
      ...prev,
      inn: data.inn,
      kpp: data.company.kpp,
      ogrn: data.ogrn,
      opf: data.company.opf,
      shortName: data.company.company_names.short_name,
      fullName: data.company.company_names.full_name,
      reversedShortName: data.company.company_names.reversed_short_name,
      managers: {
        fio: data.company.managers.length > 0 ? data.company.managers[0].fio : '',
        position: data.company.managers.length > 0 ? data.company.managers[0].position : ''
      },
      address: {
        lineAddress: data.company.address.line_address,
        zipCode: data.company.address.zip_code
      },
      // contacts: {
      //   phone: data.company.contacts.phones.length > 0 ? data.company.contacts.phones[0] : '',
      //   email: data.company.contacts.emails.length > 0 ? data.company.contacts.emails[0] : '',
      // }
    }));
  }, []);
...
useEffect(() => {
    if (partnerData) {
      cangeFormData(partnerData);
    }
  }, [partnerData, cangeFormData]);
      ...
      <form action="">
        ...
        {contacts &&
          <div className={cls.partner_info__list}>
            ...
            <div className={cls.partner_info__item}>
              <label className={cls.partner_info__label} htmlFor="legalAddress">Юридический адрес</label>
              <input defaultValue={formData.address.lineAddress} className={cls.partner_info__input} type="text" id="legalAddress" name="legalAddress" />
            </div>
            <div className={cls.partner_info__item}>
              <label className={cls.partner_info__label} htmlFor="postAddress">Почтовый адрес</label>
              <input defaultValue={formData.address.lineAddress} className={cls.partner_info__input} type="text" id="postAddress" name="postAddress" />
            </div>
            <div className={cls.partner_info__item}>
              <label className={cls.partner_info__label} htmlFor="phone">Телефон</label>
              <input className={cls.partner_info__input} value={''} onChange={handleInputChange} type="text" id="phone" name="phone" />
